...
ss_URL = (
    "https://cu3gxquyxb.execute-api.eu-west-1.amazonaws.com/dev/sentences-similarity"
)

###############   AWS API Sentences Similarity   ###############
query_params = {
    "user_input": "activation",
    "true_sentence": "activation",
}
try:
    response = requests.get(ss_URL, params=query_params, timeout=3)
    print("AWS lambda Sentences Similarity function activated!")
except Exception as e:
    print(f"AWS lambda Sentences similarity activation failed: {e}")
################################################################
...
if __name__ == "__main__":
    ...
    if embed_file:
        questions_to_embed = []
        with open("./data/basic_q&a.txt", "r") as f:
            for line in f:
                # print(line.split("\t"))
                questions_to_embed.append(line.split("\t")[0])
                # print(line.split("\t")[0])

        # print(questions_to_embed)

        # saving list of questions for future retrieval
        with open("./data/questions.txt", "w") as wf:
            for question in questions_to_embed:
                wf.writelines(question + "\n")

        model_name_or_path = "/home/solidsnake/ai/Golden_Group/ai-models/development/sentence-emebeddings/all-mpnet-base-v2/"
        model = SentenceTransformer(model_name_or_path, device="cuda")
        # Sentences are encoded by calling model.encode()
        embeddings = model.encode(questions_to_embed)

        print(f"\nembeddings shape: {embeddings.shape}")

        with open(f"./data/q&a_embeddings.npy", "wb") as fp:
    ...
    if search_in_faiss:
        ...
        # verify if index is trained
        print(f"is index trained? {index.is_trained}")

        # add emebddings to FAISS index
        index.add(qa_embeddings)
        print(f"embeddings total indexes: {index.ntotal}")

        model_name_or_path = "/home/solidsnake/ai/Golden_Group/ai-models/development/sentence-emebeddings/all-mpnet-base-v2/"
        model = SentenceTransformer(model_name_or_path, device="cpu")

        sentence_to_search = "do you have sons ?"

        start = time.time()
        xq = model.encode([sentence_to_search])
        # print(xq)

        # k is the number of similar vectors returned
        k = 1
        ...
        query_params = {
            "user_input": sentence_to_search,
            "true_sentence": from_faiss,
        }
        try:
            response = requests.get(ss_URL, params=query_params, timeout=3).json()
            prob = float(response["similarity"])
        except Exception as e:
            print(f"AWS lambda Sentences similarity activation failed: {e}")
